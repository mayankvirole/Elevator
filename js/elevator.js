(function () {
	...
	CompartmentModel = class CompartmentModel {
			constructor() {
					...
					buttons = ((function () {
							...
							for (floor = j = me.floors; j >= 1; floor = j += -1) {
									upDownButtons
									.push(`
									<div id = 'floor-buttons-${floor}' class='floor-buttons d-flex align-items-center'>
									<button class='button upSide' data-floor='${floor}'><div class='upSide'></div></button>
									<button class='button downSide' data-floor='${floor}'>
									<div class='downSide'></div></button></div>
									`);
							}
			...
			moveCompartment(compartment, floor) {
					...
					$(`#lift${compartment} .compartment`).animate({
							...
							duration: 1000 * Math.abs(myCompartment[compartment - 1].floor - floor),
							easing: 'linear',
							complete: function () {
	...
	for (let i = 0; i < view.compartmentCount; i++) {
			...
			let count = i;
			dynamicCompartment = `<div id = "lift${count+1}" class="elevator col d-flex justify-content-center"><div class="compartment"><div><div>3</div><div>2</div><div>1</div></div></div></div >`;
